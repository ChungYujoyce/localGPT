...
image = Image.open("TEST_IMAGE/page_11.png").convert("RGB")
...
reader = easyocr.Reader(['en']) # this needs to run only once to load the model into memory #dbnet18

# from manga_ocr import MangaOcr
# import unicodedata
# mocr = MangaOcr()
# image = Image.open('v1.png').convert("RGB")
...
def apply_ocr(cell_coordinates):
    ...
    for idx1, row in enumerate(tqdm(cell_coordinates)):
        ...
        for idx2, cell in enumerate(row["cells"]):
            ...
            ## mocr
            # img = cropped_table.crop(cell["cell"])
            # text = mocr(img)
...
for i in range(len(tables_crops)):
    ...
    #     tmp += row_data
    # tables.append(tmp)
    with open(f'./output_{i+1}.csv','w') as result_file:
